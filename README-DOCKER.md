...
├── Dockerfile              # 网关镜像（预编译二进制，amd64/arm64）
...
│   ├── cmd/yyb-go/         # 主程序入口
│   ├── internal/
│   │   ├── httpapi/        # 路由层：/login /scan /wxapp/* /wx/* 统一分发（fusion.go）
│   │   ├── protocol/       # 原生微信登录协议（login_buffer / pool / mmtls）
│   │   ├── qr/             # 扫码登录二维码生成与轮询
│   │   └── store/          # 账号与登录态存储（SQLite）
│   └── ...
└── gateway/
    ├── yyb-go-amd64        # Linux/amd64 预编译网关
    ├── yyb-go-arm64        # Linux/arm64 预编译网关
...
| http://<host>:8088/login?appId=wx... | 取码接口（原生扫码协议线格式；多账号自动选默认账号，`&ref=` 指定账号） |
| http://<host>:8088/health | 健康检查 |
| http://<host>:8088/instances | 账号实例列表 |

### 取码接口示例

```bash
# 1) POST /login：带 app_id 取码；带 ref 指定账号，缺省自动选默认账号
...
# 2) GET /login?appId= 兼容线格式（err/msg/appId/status/code/codeType/codeLength），客户端无需改动
...
普通用户访问这些页面会被重定向到工作台 `/`，接口返回 403；界面上也不再出现任何管理入口（v11 起）。
...
取码类接口（`/login` 取码分支、`/instances`、`/wxapp/*`、`/wx/*`、
`/openapi.json`）**默认就必须带访问令牌**（fail-closed）。
...
- **点「运行账号」会跑哪些账号？** 只会跑你选中的那一个。网关创建的账号任务在任务前命令里把 `WECHAT_OPENIDS` 覆写成该账号的 openid，脚本（v11 起，顺丰中秋 / sfsy日常版 / 移动云盘 三个脚本及 `code脚本/` 下的全部脚本）会按这个列表过滤账号，**一个都不会多跑**；白名单里没有可匹配账号时脚本直接空跑结束，绝不回退到「跑全部」。
